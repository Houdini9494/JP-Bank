...
const error = computed(() => store.getters['accounts/error'])
...
const handleSubmit = async () => {
  ...
  try{
    ...
    if (err.response && err.response.status === 409) {
      errorMessage.value = err.response.data.message || 'Errore durante la creazione del conto.'
    } else {
      errorMessage.value = 'Errore imprevisto durante la creazione del conto.'
    }
  }
}
</script>
