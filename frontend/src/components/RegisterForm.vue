...
// recupera lo stato degli errori e del caricamento dal modulo auth
const error = computed(() => store.getters['auth/getError'])
const isLoading = computed(() => store.getters['auth/isLoading'])
// gestione dell'errore locale
...
const handleSubmit = async () => {
    ...
    try {
      ...
      if (err.response && err.response.status === 409) {
        localError.value = err.response.data.message || 'Email già in uso'
      } else {
        localError.value = 'Errore imprevisto durante la registrazione.'
      }
    }
}
</script>
