...
const actions = {
  ...
  async fetchAccounts({ commit }) {
    ...
    try {
      ...
      commit('SET_ERROR', 'Errore nel caricamento dei conti')
      commit('SET_ACCOUNTS', [])
    } finally {
  ...
  async createAccount({ commit }, accountData) {
    ...
    try {
      ...
      commit('SET_ERROR', 'Errore nella creazione del conto')
      throw error
    } finally {
      commit('SET_LOADING', false)
    }
  ...
  async closeAccount({ commit, dispatch }, accountId) {
    ...
    try {
      ...
      commit('SET_ERROR', 'Errore durante la chiusura del conto')
      return false
    } finally {
      commit('SET_LOADING', false)
    }
